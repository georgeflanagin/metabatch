...
@trap
def write_slurm_to_file(slurm_dct: dict) -> None:
    """
    Rewrites the slurm file based on a dictionary.
    """
    with open("anagrammar-mod.slurm", "w") as slurm_mod_file:
...
def write_slurm_main(myargs:argparse.Namespace) -> int:
    slurm_dct = parse_slurm_file('anagrammar.slurm')
    #print(slurm_dct)
    write_slurm_to_file(slurm_dct)
    return os.EX_OK
