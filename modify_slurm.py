...
def modify_slurm_file(file: object) -> dict:
    ...
    dir_path = "/usr/local/sw/metabatch/metabatch.d/config.d/" 
    ...
    slurm_dct_mod[(len(slurm_dct)+1, "comment")] = f"#modified by metabatch on {datetime.now()}"
    slurm_dct_mod[(len(slurm_dct)+2, "comment")] =f"#metabatch version {time.ctime(os.path.getctime('metabatch.py'))}" 
